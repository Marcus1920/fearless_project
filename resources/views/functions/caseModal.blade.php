<script>
  ...
  $(document).ready(function() {
	  ...
	  $("#CreateCaseAgentForm #description").focus(function() {
		  var name = $("#CreateCaseAgentForm #name").val();
		  var surname = $("#CreateCaseAgentForm #surname").val();
		  var message = "You are referring this case to " + name + " " + surname + ". Please provide an additional message.";
		$("#message_id").removeClass("hidden");
		$("#CreateCaseAgentForm #message_header").html(message);
	  });
	  /////////// #CREATECASEAGENT END OF DEPARTMENT, CATEGORY, SUB CATEGORY AND SUB SUB CATEGORY  DROP DOWNS //////////////////////
	   $("#CreateCaseAgentForm #department").change(function(){

          $("#firstRespondersTableBody").empty();

          $.get("{{ url('/api/dropdownDepartment/category/department')}}",
          { option: $(this).val()},
          function(data) {
            ...
            if ($.isArray(data))
            {
              ...
              $("#categories_id").addClass("hidden");
              $("#categories_id").addClass("hidden");
            }
             if ($.isPlainObject(data)) {

              $("#categories_id").removeClass("hidden");
              $('#CreateCaseAgentForm #category').empty();
              $('#CreateCaseAgentForm #category').append("<option value='0'>Select Category</option>");
              $.each(data, function(key, element) {
              $('#CreateCaseAgentForm #category').append("<option value="+ key +">" + element + "</option>");
              });
    ...
    $("#CreateCaseAgentForm #category").change(function(){

          $("#firstRespondersTableBody").empty();

          $.get("{{ url('/api/dropdownDepartment/sub_category/category')}}",
          { option: $(this).val()},
          function(data) {
            ...
            if ($.isArray(data))
            {
              ...
              $("#sub_categories_id").addClass("hidden");
            }
             if ($.isPlainObject(data)) {

              $("#sub_categories_id").removeClass("hidden");
              $('#CreateCaseAgentForm #sub_category').empty();
              $('#allocationCaseForm #sub_category').append("<option value='0'>Select Category</option>");
              $.each(data, function(key, element) {
                $('#CreateCaseAgentForm #sub_category').append("<option value="+ key +">" + element + "</option>");
              });
	 ...
	 $("#CreateCaseAgentForm #sub_category").change(function(){

          $("#firstRespondersTableBody").empty();

          $.get("{{ url('/api/dropdownDepartment/sub_sub_category/sub_category')}}",
          { option: $(this).val()},
          function(data) {
            ...
            if ($.isArray(data))
            {
              ...
              $("#sub_sub_categories_id").addClass("hidden");
              $("#sub_sub_categories_id").addClass("hidden");
            }
             if ($.isPlainObject(data)) {

              $("#sub_sub_categories_id").removeClass("hidden");
              $('#CreateCaseAgentForm #sub_sub_category').empty();
              $('#CreateCaseAgentForm #sub_sub_category').append("<option value='0'>Select Category</option>");
              $.each(data, function(key, element) {
              $('#CreateCaseAgentForm #sub_sub_category').append("<option value="+ key +">" + element + "</option>");
              });
	 ...
	 /////////// END OF DEPARTMENT, CATEGORY, SUB CATEGORY AND SUB SUB CATEGORY  DROP DOWNS //////////////////////
	  
	  
	  
	  //$("#modalReferCase").modal({backdrop: 'static', keyboard: false})
	  
	  $('#modalDueDate #dueDate').timeEntry();

	  $("#modalDueDate #dueModalSave").click(function(){
			var time = $("#modalDueDate #defaultEntry").val();
			var date = $("#modalDueDate #dueDate").val();
			
			$("#modalReferCase #dueTimeGroup").show();
			$("#modalReferCase #dueTime").val(time);
			
			$("#modalReferCase #dueDateGroup").show();
			$("#modalReferCase #dueDate").val(date);
	  ...
	  $("#openDialog").click(function(){
		...
		$("#dialogSave").click(function(){
			var time = $("#dialogTime").val();
			var date = $("#dialogDate").val();
...
$("#add_case_search").tokenInput("{!! url('/getCaseSearch')!!}", {
 ...
 var oTable     = $('#casesTable').DataTable({
				...
				{data: function(d){
					...
					var year = d.due_date.substring(0,4);
					var month =  d.due_date.substring(5,7);
					var day =  d.due_date.substring(8,10);
					...
					if(daysToGo < 0){
						returnString = "<div data-togle='tooltip' title='Was due " + realDaysGo+ now + " day(s) ago'  style='background-color:#b71c1c; padding-top: 1px; padding-bottom: 1px; cursor: pointer;'> <p style='color: #fff; margin-left: 30px; margin-top: 1px; font-size: 13px; font-family: Tahoma;'>"+d.due_date+"</p> </div>";
					}else if(daysToGo == 0){
						...
						returnString = "<div data-togle='tooltip' title='Due in " + realDaysGo + " day(s).' style='background-color:#4caf50; padding-top: 1px; padding-bottom: 1px; cursor: pointer;'> <p style='color: #fff; margin-left: 30px; margin-top: 1px; font-size: 13px; font-family: Tahoma;'>"+d.due_date+"</p> </div>";
					}else{
						returnString = "<div data-togle='tooltip' title='Due in " + realDaysGo + " day(s)' style='background-color:#4caf50; padding-top: 1px; padding-bottom: 1px; cursor: pointer;'> <p style='color: #fff; margin-left: 30px; margin-top: 1px; font-size: 13px; font-family: Tahoma;'>"+d.due_date+"</p> </div>";
					}
					...
					//return "<div data-togle='tooltip' title='Was due " + diffDays + firstDate + " day(s) ago'  data-toggle='modal' data-target='.modalDueDate' style='background-color:#b71c1c; padding-top: 1px; padding-bottom: 1px; cursor: pointer;'> <p style='color: #fff; margin-left: 30px; margin-top: 1px; font-size: 13px; font-family: Tahoma;'>"+d.due_date+day+"</p> </div>";

                },"name" : 'cases.due_date'},
				
               {data: 'source', name: 'cases_sources.name'},

                     {data: 'CasePriority', name: 'cases_priorities.name',"visible" :true},
   ...
   , 'initComplete': function() {
                	...
                	this.api().columns().every(function() {
                		var col = this;
                		var skip = false;
                		skip = col.header().innerText.search(/PRIORITY/);
										console.log("  every() col- ",col.header().innerText,", skip - ",skip);
										var select = $('<select class="form-control input-sm" id="selPriority"><option value="">Select Priority</option></select>')
											//.appendTo( $('.dataTables_length') );
                      .on("change", function() {
												console.log("selPriority.on");
                          ...
                          .draw();
										...
										//if (skip == 0) select.appendTo( $('.dataTables_length') );

										if (skip == 0) col.data().unique().sort().each( function(d,j) {
                			...
                			select.append('<option value="'+d+'">'+d+'</option>');
										...
										if (skip == 0) {
											//$('&nbsp;&nbsp;&nbsp;').appendTo( $('.dataTables_length') );
                      var wSkip = $('<div></div>');
                      wSkip.css("float", "right");
											$('<label>Priority:&nbsp;&nbsp;</label>').appendTo(wSkip);
											select.appendTo( wSkip );
											wSkip.appendTo($('.dataTables_length'));
                    }
...
$("#submitAssociatePoiForm").on("click",function(){
  ...
  $("#submitCreateCaseAgentForm").on("click",function(){
		...
		var due_date                    = $("#CreateCaseAgentForm #due_date").val();
        var due_time                    = $("#CreateCaseAgentForm #due_time").val();
		var estimatedDate               = $("#CreateCaseAgentForm #estimatedDate").val();
		var estimateTime                = $("#CreateCaseAgentForm #estimateTime").val();
		var message		                = $("#CreateCaseAgentForm #message").val();
		var department                  = $("#CreateCaseAgentForm #department option:selected").text();
        var category               		= $("#CreateCaseAgentForm #category option:selected").text();
		var sub_category                = $("#CreateCaseAgentForm #sub_category option:selected").text();
        var sub_sub_category            = $("#CreateCaseAgentForm #sub_sub_category option:selected").text();
      ...
      $.ajax({
          ...
          error: function(data) {
              ...
              HoldOn.close();
               ...
               $("#hse_error_due_date").html("");
               $("#hse_error_estimated_due_date").html("");
               $("#hse_error_due_time").html("");
               ...
               if (data.responseJSON.due_date)
               {
               $("#hse_error_due_date").html("<p class='help-block red'>*"+data.responseJSON.due_date+"</p>")
               }


               if (data.responseJSON.estimatedDate)
               {
               $("#hse_error_estimated_due_date").html("<p class='help-block red'>*"+data.responseJSON.estimatedDate+"</p>")
               }
     ...
     $("#submitEscalateCaseForm").on("click",function(){
		...
		var dueDate 	= $("#modalReferCase #dueDate").val();
		var dueTime		= $("#modalReferCase #dueTime").val();

        var token     = $('input[name="_token"]').val();
        var formData  = { addresses:addresses,message:message,caseID:caseID,modalType:modalType,dueDate:dueDate,dueTime:dueTime};
        ...
        success :function(data) {
           ...
           if(data == 1)
           {
            ...
            $.ajax({
                  ...
                  if(data!== null)
                  {
                      if (data.status.name == "Pending Closure" || data.status.name == "Resolved" ) {

                         $( "#requestCaseClosureClass" ).addClass( "hidden" );
                      }

                      if (data.status.name == "Resolved" ) {

                         $( "#closeCaseClass" ).addClass( "hidden" );
                      }

                     $("#modalCase #id").val(data.id);
                     $("#modalCase #created_at").val(data.created_at);
                     $("#modalCase #description").val(data.description);
                     $("#modalCase #case_type").val(data.case_type);
                     $("#modalCase #dueDate").val(data.due_date);
                     $("#modalCase #category").val(data.case_type);
                     $("#modalCase #sub_sub_category").val(data.case_sub_type);
                     $("#modalCase #status").val(data.status.name);
                     $("#modalCase #department").val(data.department.name);
                     $("#modalCase #province").val(data.province);
                     $("#modalCase #district").val(data.district);
                     $("#modalCase #municipality").val(data.municipality);
                     $("#modalCase #ward").val(data.ward);
                     $("#modalCase #street_number").val(data.street_number);
                     $("#modalCase #route").val(data.route);
                     $("#modalCase #locality").val(data.locality);
                     $("#modalCase #administrative_area_level_1").val(data.administrative_area_level_1);
                     $("#modalCase #postal_code").val(data.postal_code);
                     $("#modalCase #country").val(data.country);
                     $("#modalCase #driver").val(data.vehicleDriver);
                      $("#modalCase #vehicleReg").val(data.vehicleReg);
                      $("#modalCase #vehiclePurpose").val(data.vehiclePurpose);
                     $("#modalCase #launchUpdateUserModalField").attr("data-id",data.reporteID);
                     $("#modalCase #launchUpdateUserModalHouse").attr("data-id",data.house_holder_id);

                     if (data.img_url) {

                        var ImgUrl = "http://154.0.164.72:8080/moblile_backend/Port_Beckend_Mobile/public/"+data.img_url;
                        $("#modalCase #CaseImageA").attr("href",ImgUrl);

                     }

                    if (data.house_holder_id < 1 && data.status.name =='Pending') {
                    ...
                    if (data.status.name == 'Referred') {
                     ...
                     $("#modalCase #reporter").val(data.reporter);
                     $("#modalCase #reporterCell").val(data.reporterCell);
                     $("#modalCase #reporterPosition").val(data.reporterPosition);
                     $("#modalCase #household").val(data.household);
                     $("#modalCase #householdCell").val(data.householdCell);
                     $("#modalCase #client_reference_number").val(data.reference_number);
                     $("#modalCase #sapc_number").val(data.saps_case_number);
                     $("#modalCase #investigation_officer").val(data.investigation_officer);
                     $("#modalCase #investigation_cell").val(data.investigation_cell);
                     $("#modalCase #investigation_email").val(data.investigation_email);
                     $("#modalCase #investigation_note").val(data.investigation_note);
                     $("#modalCase #saps_station").val(data.saps_station);
                     $("#modalCase #priority").val(data.priority);
        ...
        success :function(data) {
            ...
            if(data[0] !== null)
            {
              ...
              $("#modalEditUser #ward").attr("disabled","disabled");
               ...
               $("#modalEditUser #port").val(data[0].port);
