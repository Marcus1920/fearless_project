...
class UserController extends Controller
{
    ...
    public function getResponders(Request $request)
    {
        ...
        if (sizeof($subSubCategory) > 0 && $subSubCategory != '0') {
          ...
          $objSubSubCategory     = CaseSubSubType::where('slug','=',$subSubCategory)->first();
          $objSubCategory        = CaseSubType::find($objSubSubCategory->id);
          $objCaseResponder      = CaseResponder::where('case_type','=',$objSubCategory->category)
                                                  ->where('case_sub_type','=',$objSubSubCategory->sub_category)
                                                  ->where('case_sub_sub_type','=',$objSubSubCategory->id)
                                                  ->first();
        ...
        if (sizeof($subCategory) > 0 && sizeof($subSubCategory) == 0) {
            ...
            $objSubCategory = CaseSubType::where('slug','=',$subCategory)->first();


            $allSubsCats    = CaseSubType::where('name','=',$objSubCategory->name)->get();
            ...
            foreach ($allSubsCats as $sub_sub_cat) {
                ...
                $objCaseResponder   = CaseResponder::where('case_type','=',$sub_sub_cat->category)
                                            ->where('case_sub_type','=',$sub_sub_cat->id)
                                            ->first();
    ...
    public function construct_poi_case_associate_object($CasePoiAssociate) {
        ...
        $userObj             = new \stdClass();
        $userObj->id         = "m-".$CasePoiAssociate->case_id;
        ...
        $linkObj->to         = "m-". $CasePoiAssociate->case_id;
      ...
      $links                = array();
        ...
        if(sizeof($objCasePoiAssociates) > 0) {
            ...
            foreach ($objCasePoiAssociates as $CasePoiAssociate) {
                  ...
                  $nodes[]          = array_merge($poi_associates['nodes'],$responseNodes);
            ...
            foreach ($links as $link) {
              ...
              $response["nodes"] = array_merge($n);
              $response["links"] = array_merge($l);
  ...
  public function construct_case_associate_objects($CasePoiAssociate) {
        ...
        $linkObj             = new \stdClass();
        $linkObj->id         = $poiObj->id;
        $linkObj->from       = "m-". $CasePoiAssociate->case_id;
        $linkObj->to         = "m-". $poiObj->id;
        $linkObj->type       = "Associated";
        $response["nodeObj"] = $userObj;
        $response["linkObj"] = $linkObj;
       ...
       if ($exception > 0) {
        ...
        if(sizeof($objAssociates) > 0) {
            ...
            foreach ($objAssociates as $associate) {
                ...
                $response["nodes"] = array_merge($responseNodes,$associates['nodes']);
                $response["links"] = array_merge($responseLinks,$associates['links']);
    ...
    public function edit($id,User $user)
    {
      ...
      $user = \DB::table('users')
        ...
        if ($userObj->port_id > 0) {

            $port          = Port::find($userObj->port_id);
            $user->port    = $port->id;
